...
test('fracciones: formato, enteros, ceros y equivalencia exacta', () => {
  ...
  assert(E.same([2, 4], [1, 2]));
  assert(!E.same([1, 4], [1, 2]));
});
test('el banco completo genera resultados matemáticos válidos', () => {
  for (const skill of E.skills)
    for (let i = 0; i < 100; i++) {
      const t = E.makeTask(skill.id, i);
      assert(t.answer[1] > 0);
      assert(t.answer[0] >= 0);
      if (t.b) {
        const expected = [
          t.a[0] * t.b[1] + (t.op === '−' ? -1 : 1) * t.b[0] * t.a[1],
          t.a[1] * t.b[1],
        ];
        assert(E.same(t.answer, expected));
      }
...
test('una respuesta con ayuda no aumenta ni reduce dominio', () => {
  const initial = { p: 0.5, n: 0, correct: 0 };
  ...
  const one = E.updateKnowledge(initial, true, false);
  assert(!E.mastered(one));
  const three = E.updateKnowledge(E.updateKnowledge(one, true, false), true, false);
  assert(E.mastered(three));
...
test('el selector respeta prerrequisitos y no fuerza habilidades avanzadas por edad', () => {
  const k = E.newKnowledge();
  assert.equal(E.nextSkill(k), 'meaning');
  k.meaning = { p: 0.99, n: 3, correct: 3 };
  for (let i = 0; i < 20; i++) {
    const id = E.nextSkill(k),
      skill = E.skills.find((s) => s.id === id);
...
test('la ayuda automática no se interpreta como preferencia', () => {
  const events = Array.from({ length: 20 }, () => ({
    type: 'support_selected',
    data: { mode: 'audio', voluntary: false },
  }));
...
test('triangulación 60/40 solo con vector familiar válido', () => {
  const events = Array.from({ length: 8 }, () => ({
    type: 'support_selected',
    data: { mode: 'visual', voluntary: true },
  }));
  const parent = { estructurado: 1, visual: 1, auditivo: 6, explorador: 2 };
  const p = E.inferProfile(events, parent);
  assert.equal(p.provisional, false);
  assert(Math.abs(p.combined.visual - (0.6 * p.behavior.visual + 0.4 * 0.1)) < 1e-12);
...
test('las pausas y ayudas modifican la duración sugerida, no el conocimiento', () => {
  const high = E.sessionPolicy([]),
    low = E.sessionPolicy(Array.from({ length: 4 }, () => ({ type: 'hint_requested' })));
  assert(low.breakAfterMs < high.breakAfterMs);
  assert.equal(low.blockMs, 60000);
});
...
test('los cuatro perfiles se infieren con evidencia de contextos distintos', () => {
  for (const profile of E.profiles) {
    ...
    assert.equal(result.dominant, profile);
...
test('evidencia reciente puede cambiar la preferencia y la ayuda automática no tapa la voluntaria', () => {
  const events = Array.from({ length: 90 }, (_, i) => ({
    type: 'experience_selected',
    task: `task-${i}`,
    data: { profile: i < 10 ? 'visual' : 'auditivo' },
  }));
  assert.equal(E.inferProfile(events, null).dominant, 'auditivo');
  assert.equal(
    E.inferProfile(
      [
        { type: 'support_selected', task: 'a', data: { mode: 'audio', voluntary: false } },
        { type: 'support_selected', task: 'a', data: { mode: 'audio', voluntary: true } },
      ],
      null,
    ).observations,
    1,
